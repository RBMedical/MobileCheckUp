const APP_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwE6h12pZPWdutqHyN4ugstNHZxxr6E8bkYg2dJEshz79MD0Ifd7IHajg1Wl1XFJB4k/exec';
const fields = [
  'HN',
  'ชื่อ นามสกุล',
  'รหัสประจำตัว',
...
async function processBarcode() {
  ...
  try {
    ...
    statusEl.className = 'specimen-barcode-status err';
    return setSpecimenStatus('เชื่อมต่อไม่สำเร็จ', false);
  }

  if (!result.ok) {
    statusEl.textContent = result.message || 'บันทึกไม่สำเร็จ';
    statusEl.className = 'specimen-barcode-status err';
    return setSpecimenStatus(result.message || 'บันทึกไม่สำเร็จ', false);
  }
